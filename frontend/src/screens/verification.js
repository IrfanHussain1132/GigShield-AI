...
<div class="min-h-full bg-[#fdfcfb] flex flex-col pb-32">
      ...
      <div class="space-y-4">
        ...
        <div class="relative group">
          <input id="partner-id-input" aria-label="Partner ID Number" 
            ...
            oninput="state.partnerId = this.value.trim(); const iv = /^[A-Z]{2}-[0-9]{4,6}$/.test(this.value.toUpperCase()) || this.value.toLowerCase() === 'admin1234'; const b = document.getElementById('btn-verify'); if(b) { b.disabled = !iv; b.style.opacity = iv ? '1' : '0.5'; } "/>
  ...
  <footer class="sticky-bottom-action">
    <button id="btn-verify" onclick="actions.verifyPartner()" ${(state.partnerId && (/^[A-Z]{2}-[0-9]{4,6}$/.test(state.partnerId.toUpperCase()) || state.partnerId.toLowerCase() === 'admin1234')) ? '' : 'disabled style="opacity: 0.5;"'} class="btn btn-primary h-20 rounded-[28px] shadow-2xl shadow-primary/30 pointer-events-auto active:scale-95 transition-transform flex items-center px-8">
