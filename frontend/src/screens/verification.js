...
<div class="min-h-full bg-surface flex flex-col pb-24">
  ...
  <section class="px-6 py-4 space-y-4">
    <h2 class="font-headline font-bold text-3xl text-on-surface">${t('select_platform')}</h2>
    <div class="flex gap-2">
      ...
      <button data-plat="both" onclick="actions.selectPlatform('both')" class="plat-btn flex-1 py-3 px-4 rounded-xl ${state.platform === 'both' ? 'bg-primary-container text-on-primary-container shadow-sm' : 'bg-surface-container-low text-on-surface-variant'} font-medium text-sm transition-all">${t('both')}</button>
    </div>
  </section>

  <section class="px-6 py-4 space-y-4">
    <div class="space-y-2">
      <label class="text-xs font-bold text-outline uppercase tracking-widest" for="partner-id-input">${t('partner_id_label')}</label>
      <div class="relative">
    ...
    <details class="group bg-surface-container-low rounded-xl overflow-hidden transition-all">
      <summary class="flex items-center justify-between p-4 cursor-pointer list-none">
        <span class="text-sm font-semibold text-on-surface flex items-center gap-2"><span class="material-symbols-outlined text-sm">help</span>${t('where_find')}?</span>
        <span class="material-symbols-outlined transition-transform group-open:rotate-180">expand_more</span>
      </summary>
      <div class="px-4 pb-4 text-xs text-on-surface-variant leading-relaxed">${t('find_id_desc') || 'Open your Partner App, tap profile, and look for the unique alphanumeric code below your name.'}</div>
    </details>
  </section>
  ...
  <section class="px-6 py-4">
    <div class="bg-surface-container-low rounded-[28px] p-8 border-2 border-dashed border-outline-variant/30 flex flex-col items-center justify-center text-center">
      ...
      <h3 class="font-headline font-bold text-on-surface mb-1">${t('awaiting_partner_id')}</h3>
      <p class="text-on-surface-variant text-xs max-w-[200px]">${t('verification_score_calc_desc') || 'Verification score will be calculated once your ID is successfully matched.'}</p>
    </div>
  </section>

  <footer class="mt-auto px-6 py-8">
    <button onclick="actions.verifyPartner()" class="btn btn-primary">
      ${t('verify_partner_id')} <span class="material-symbols-outlined">arrow_forward</span>
    </button>
  </footer>
</div>`;
}
...
<div class="min-h-full bg-surface flex flex-col pb-24">
  ...
  <section class="px-6 py-4">
    <div class="bg-secondary-container/10 p-5 rounded-xl flex items-center justify-between">
      <div class="space-y-1">
        ...
        <p class="font-noto text-2xl font-black text-on-secondary-container">${formatCurrency(potential)}</p>
      </div>
  ...
  <footer class="mt-auto px-6 py-8">
    <button onclick="actions.goToPremium()" class="btn btn-primary">
      ${t('looks_right_continue')} <span class="material-symbols-outlined">arrow_forward</span>
    </button>
  </footer>
</div>`;
}
